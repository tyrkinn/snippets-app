...
const Editor = ({}) => {
  const { categoryList, setEditorCategory, editorCategory, addSnippet, editorValue, setEditorValue, updateSnippetList } = useStore(
    (state) => ({
  ...
  const onAddSnippet = async () => {
    ...
    if (!editorNotChosen && !inpEmpty && !nameEmpty) {
      const res = await addSnippet({ name: tname, code: tinp, category: editorCategory });
      if (res) {
    ...
    >
      ...
      <GridItem>
        <HStack>
          ...
          />
          <Input placeholder="Snippet name..." w="70%" value={nameInp} onChange={e => setNameInp(e.target.value)}/>
        </HStack>
      </GridItem>
