...
const SnippetItem = ({ item }) => {
  const {deleteSnippet, updateSnippetList}  = useStore(state => ({deleteSnippet: state.deleteSnippet, updateSnippetList: state.updateSnippetList }));
  ...
  const onCopy = async (e) => {
    ...
    if (res) {
      toast({status: 'success', title: 'Successfully copied code into clipboard'});
    } else {
  ...
  return (
    ...
    >
      ...
      <ButtonGroup>
        <Button variant="link" colorScheme='telegram' onClick={onDeleteSnippet}>Delete</Button>
      </ButtonGroup>
    </HStack>
  );
};
